...
class XiaoYangTou {
  ...
  constructor(path){
    this.path = path;
    let pathExist = fs.existSync(path);
    if(!pathExist)
    ...
    return new Promise( (resolve, reject) => {
      ...
      incomingMessage.on('data', chunk => {
        if(state == 0){
          ...
          // 如果不是第一次，就把上次的数据存起来
          fs.writeFileSync('C:\\Users\\37230\\workspace\\mine\\static\\haha.png', last, {
            flag: 'a'
          });
          last = chunk;
        }
      ...
      incomingMessage.on('end', () => {
        for(let i=last.length-5; i>0; i--){
          if(last[i]==10&&last[i-1]==13){
            last = last.slice(0, i-1);
            break;
          }
        }
        fs.writeFileSync('C:\\Users\\37230\\workspace\\mine\\static\\haha.png', last, {
